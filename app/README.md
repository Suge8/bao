...
当前正在查看的 active 会话若因外部新消息触发排序更新，侧边栏不会再为了把该会话提到更前面而把列表打回顶部；侧栏最终行模型（header/session/child + active pinning + 展开态 + 未读/运行聚合）已下沉到 `SessionService.sidebarModel`，QML 不再二次拼装 `groupModel`。折叠分组只保留 header row，session 行不会再以 0 高度幽灵行形式留在 `ListView` 里，从根上减少底部快速滚动时的虚拟化抖动。
...
侧边栏分组头现在支持最小吸顶：当用户在展开分组内部向下滚动时，顶部会显示当前分组的只读 sticky header；它完全派生自同一份 `SessionService.sidebarModel`，仅复用现有 toggle 动作，不引入第二套分组状态或额外滚动控制逻辑。sticky header 的可见边界也收口在列表顶部裁剪视口内，被下一个分组顶走时只会在列表内退出，不会越过到会话标题区。

会话运行态也收敛到单一路径：`SessionManager` 现在把 `session_running` 与 `child_status=running` 视为当前进程的 runtime overlay，只在内存里随事件更新，并在 `list_sessions()` / `_load()` 时与稳定 metadata 合并投影；主代理、桌面网关与子代理分别在自己的编排边界显式推送/清理 runtime running 事件，completed/failed/cancelled 这类稳定结果仍正常持久化。`SessionService` 只负责消费这条单一路径并聚合成列表项/分组头的 running badge，QML 不再自己推断 busy 状态，进程重启后也不会把旧 running 残留重新读出来。
...
会话删除体验已收敛到单一路径：Sidebar 点击删除只发送意图，成功/失败 toast 统一由 `deleteCompleted` 回包决定；本地乐观删除命中的 `deleted` 提交事件会直接复用当前删除事务，不再触发第二次列表重建。若删除已落盘、只是后续 active marker 同步失败，`SessionService` 也会直接按持久化事实刷新收口，不把已删会话从 snapshot 回滚回来。Sidebar 在列表重建前后也不再死守旧 `contentY`，而是按当前可见行锚点恢复视口，让删除上方会话时可见内容跟着稳定收口。`SessionItem` 点击命中也做了单一路径分区：删除按钮可见时主行点击区自动让出右侧区域，避免“选中会话”和“删除会话”争抢同一次 pointer 事件。
...
输入框点击焦点仅走 `TextArea` 原生路径（移除了容器级聚焦 MouseArea），首击更稳定；窗口层现在还补上了统一的 click-away 收口：只有显式声明 `baoClickAwayEditor` 的编辑控件才参与窗口级 blur，而 `SettingsSelect` 这类自带 popup 的下拉字段则通过同一个 `WindowFocusDismissFilter` 在 press 阶段处理“外点关闭但不吞首击”。这样点击位置落在编辑器外时，主窗口会清掉输入选区并把焦点从该编辑控件移走，打开中的 dropdown 也不会再把下一次按钮点击吃成“先关弹层、第二次才生效”。同一个 `MouseButtonRelease` 边界还会补一次窗口级 pointer 重算，用来处理点击引发的切页/弹层/显隐等场景变化发生在静止鼠标下时，hover/cursor owner 不能及时刷新的问题；Settings 里的按钮、渠道头和“+ 添加 LLM 提供商”首击都必须继续直接生效。
...
桌面端测试现在除最小 QML harness 外，还补了一层真实页面集成回归：直接加载 `Main.qml`（从真实对象树进入 `ChatView` / `SettingsView`），在根窗口安装 `WindowFocusDismissFilter`，验证声明了 `baoClickAwayEditor` 的输入控件会在外部点击时清焦点和选区，同时 `MouseArea`/`ExpandHeader` 这类真实 click owner 的首击不会被窗口级失焦吞掉，并继续覆盖鼠标释放后会触发窗口级 pointer refresh，避免“简化 harness 通过但真实页面回归”。
...
- `发给 Bao / Ask Bao` 仅在存在结构化诊断事件时显示，且默认只发送错误摘要，不自动附带日志尾部
