...
侧边栏快速连点切换时，当前激活会话以“用户最新选择的 session key”为事实源；`desktop:local` 与对应渠道 family 的 active marker 通过同一提交路径落盘，异步列表刷新不会回滚到旧会话。会话一旦提交到当前视图，就会立即写入 `desktop_last_seen_ai_at`，不再等待 history apply 回包后补标已读，减少“已经看过但切走后红点复活”的中间态。
...
侧边栏分组头现在支持最小吸顶：当用户在展开分组内部向下滚动时，顶部会显示当前分组的只读 sticky header；它完全派生自同一份 `SessionService.sidebarModel`，仅复用现有 toggle 动作，不引入第二套分组状态或额外滚动控制逻辑。

会话运行态也收敛到单一路径：主代理与子代理都只往 session metadata 写 `session_running` / `child_status`，`SessionService` 负责把它们投影成列表项的 running badge，并进一步在 Sidebar 分组头聚合为“该渠道下有活跃会话”的呼吸点；QML 不再自己推断 busy 状态。
