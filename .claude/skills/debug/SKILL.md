...
|-----|----------|---------|
| **Main app logs** | `logs/bao.log` | Host-side WhatsApp, routing, container spawning |
| **Main app errors** | `logs/bao.error.log` | Host-side errors |
...
echo '{}' | container run -i \
  ...
  --entrypoint /bin/bash bao-agent:latest \
...
```bash
container run --rm --entrypoint /bin/bash bao-agent:latest -c 'ls -la /workspace/'
```
...
```bash
container run --rm --entrypoint /bin/bash bao-agent:latest -c '
  whoami
  ls -la /workspace/
  ls -la /app/
'
...
```bash
container run --rm -it --entrypoint /bin/bash bao-agent:latest
```
...
# Also clear the session ID from bao's tracking (stored in SQLite)
sqlite3 store/messages.db "DELETE FROM sessions WHERE group_folder = '{groupFolder}'"
```

To verify session resumption is working, check the logs for the same session ID across messages:
```bash
grep "Session initialized" logs/bao.log | tail -5
...
```bash
echo "=== Checking bao Container Setup ==="
...
container system status &>/dev/null && echo "OK" || echo "NOT RUNNING - bao should auto-start it; check logs"

echo -e "\n4. Container image exists?"
echo '{}' | container run -i --entrypoint /bin/echo bao-agent:latest "OK" 2>/dev/null || echo "MISSING - run ./container/build.sh"
...
SESSIONS=$(grep "Session initialized" logs/bao.log 2>/dev/null | tail -5 | awk '{print $NF}' | sort -u | wc -l)
