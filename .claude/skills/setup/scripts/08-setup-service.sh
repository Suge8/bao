...
case "$PLATFORM" in
  ...
  macos)
    PLIST_PATH="$HOME_PATH/Library/LaunchAgents/com.bao.plist"
...
<dict>
    ...
    <string>${PROJECT_PATH}/logs/bao.log</string>
    <key>StandardErrorPath</key>
    <string>${PROJECT_PATH}/logs/bao.error.log</string>
</dict>
</plist>
PLISTEOF
    ...
    if launchctl list 2>/dev/null | grep -q "com.bao"; then
      SERVICE_LOADED="true"
...
EOF
  ...
  linux)
    ...
    UNIT_PATH="$UNIT_DIR/bao.service"
    mkdir -p "$UNIT_DIR"
    log "Generating systemd unit at $UNIT_PATH"

    cat > "$UNIT_PATH" <<UNITEOF
[Unit]
Description=bao Personal Assistant
...
StandardOutput=append:${PROJECT_PATH}/logs/bao.log
StandardError=append:${PROJECT_PATH}/logs/bao.error.log
...
UNITEOF
    ...
    systemctl --user enable bao >> "$LOG_FILE" 2>&1 || true
    systemctl --user start bao >> "$LOG_FILE" 2>&1 || true

    # Verify
    SERVICE_LOADED="false"
    if systemctl --user is-active bao >/dev/null 2>&1; then
      SERVICE_LOADED="true"
