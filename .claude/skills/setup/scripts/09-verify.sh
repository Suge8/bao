...
if [ "$PLATFORM" = "macos" ]; then
  if launchctl list 2>/dev/null | grep -q "com.bao"; then
    ...
    LAUNCHCTL_LINE=$(launchctl list 2>/dev/null | grep "com.bao" || true)
  ...
  if systemctl --user is-active bao >/dev/null 2>&1; then
    SERVICE="running"
  elif systemctl --user list-unit-files 2>/dev/null | grep -q "bao"; then
    SERVICE="stopped"
  fi
fi
log "Service: $SERVICE"
...
if [ -f "$HOME/.config/bao/mount-allowlist.json" ]; then
