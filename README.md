...
- **动态注入** — 技能索引会以单行摘要注入 system prompt，并携带精确 `path/source/available` 元数据，命中后直接按 `path` 读取
...
工具前短说明与 tool hint 会以用户可见的 assistant 过程提示保留在时间线中，并按会话语言输出简短文案（zh/en）；这类提示只用于展示当前动作，不会回流为后续 prompt 输入。
